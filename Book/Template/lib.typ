...
#let book(
  ...
  edition: 0,
  /** <- int
    * The book publication number; used when the content is changed or updated
   ...
   * control its appearance. They are built with rebust defaults in mind, so that
  ...
  context {
    ...
    **/
    ...
    show heading: it => {
      ...
      set align(
        if it.level >= 3 { left } else { center }
      )
    ...
    if toc == true {
      ...
      show outline.entry.where(level: 2): it => strong(emph(it))  // Bold + italic

      // show outline.entry.where(level: 2): it => strong(it)  // Make level 2 bold
      // show outline.entry.where(level: 2): it => emph(it)  // Italic for level 2
